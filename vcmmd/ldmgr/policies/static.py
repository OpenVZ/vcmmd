...
class StaticPolicy(Policy):
    ...
    def balance(self, active_ves, mem_avail, stats_updated):
        ...
        sum_lim = sum(ve.config.effective_limit for ve in active_ves)
        if sum_lim <= mem_avail:
            return {ve: ve.config.effective_limit for ve in active_ves}
        ...
        return {ve: (ve.config.guarantee + (mem_avail - sum_guar) *
                     (ve.config.effective_limit - ve.config.guarantee) /
                     (sum_lim - sum_guar + 1))
